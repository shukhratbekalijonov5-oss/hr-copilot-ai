...
/**
 * Switches between organizations for ORGANIZATION accounts, without signing
 * out. Candidate accounts are exclusive in the current identity model, so they
 * get a static label rather than a Candidate ↔ Organization switch.
 */
export function WorkspaceSwitcher({ context }: WorkspaceSwitcherProps) {
  ...
  const { active, organizations } = context;
  ...
  if (active.kind === "personal") {
    return (
      <div className="flex min-w-0 items-center gap-2 rounded-lg py-1 pl-1 pr-1.5">
  ...
  return (
    <div className="relative min-w-0" ref={containerRef}>
      <button
        ...
        className="flex min-w-0 items-center gap-2 rounded-lg py-1 pl-1 pr-1.5 hover:bg-surface-muted disabled:opacity-75"
      >
        <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-surface-muted text-[11px] font-semibold text-ink-muted">
          {pending ? (
        ...
        <span className="min-w-0 text-left">
          ...
          <span className="block truncate text-[11px] leading-tight text-ink-subtle">
            {pending ? d.workspaces.switching : d.status.role[active.role]}
          </span>
        </span>
        ...
        >
          ...
          <div className="p-1">
            {organizations.map((organization) => {
              const isActive = context.activeOrganizationId === organization.id;
              return (
                <button
                  key={organization.id}
                  type="button"
