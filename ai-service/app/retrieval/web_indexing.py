...
ONE function, because there is one place candidate evidence lives:

  * ``index_candidate_web_source``    → candidate collection, keyed by
    candidateAccountId, with NO organizationId anywhere.

Since application-time evidence snapshots were removed, nothing copies a
candidate's link into an organization's collection. A recruiter reads these
same chunks, through the backend's vacancy-contextual authorization chain —
the separation that used to be physical is now an explicit account filter on
every query.
