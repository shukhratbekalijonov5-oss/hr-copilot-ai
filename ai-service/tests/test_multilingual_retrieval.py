...
@pytest.fixture()
def english_resume(candidate_store, embedder):
    """Returns the ACCOUNT id — the key retrieval is scoped by."""
...
@pytest.fixture()
def korean_resume(candidate_store, embedder):
...
def _search(store, embedder, account, query, limit=5):
    return search_evidence(
        candidate_account_ids=[account], query=query, limit=limit,
...
class TestQueryLanguageVersusDocumentLanguage:
    ...
    def test_all_four_locales_retrieve_from_an_english_cv(
        self, candidate_store, embedder, english_resume, locale, query
    ):
        response = _search(candidate_store, embedder, english_resume, query)
    ...
    def test_english_query_retrieves_from_a_korean_cv(
        self, candidate_store, embedder, korean_resume
    ):
        """The reverse direction: English query, Korean source document."""
        response = _search(candidate_store, embedder, korean_resume, "Kubernetes production experience")
    ...
    def test_russian_query_retrieves_from_a_korean_cv(
        ...
        response = _search(candidate_store, embedder, korean_resume, "опыт работы с Redis")
...
class TestMultilingualProvenance:
    def test_citations_from_a_korean_cv_stay_in_korean(
        ...
        response = _search(candidate_store, embedder, korean_resume, "Kubernetes experience")

        text = " ".join(h.text for h in response.hits)
        assert any(ord(ch) > 0x1100 for ch in text), "Korean source text was altered"

    def test_metadata_survives_across_languages(self, candidate_store, embedder, korean_resume):
        response = _search(candidate_store, embedder, korean_resume, "경력")
...
class TestMultilingualAccountIsolation:
    """Isolation must not weaken just because the query is in another script.

    The filter is a payload key match, so language is irrelevant to it — which
    is precisely the sort of thing worth pinning, because "the query looked
    foreign" is exactly the kind of edge a hand-rolled filter gets wrong.
    """

    def test_a_foreign_language_query_cannot_cross_accounts(
        self, candidate_store, embedder, english_resume
    ):
        other = f"acct-other-{uuid.uuid4()}"
        response = _search(candidate_store, embedder, other, "프로덕션 쿠버네티스 운영 경험")

        assert response.hits == []

    def test_a_foreign_language_query_with_an_empty_universe_finds_nothing(
        self, candidate_store, embedder, english_resume
    ):
        response = search_evidence(
            candidate_account_ids=[], query="프로덕션 쿠버네티스 운영 경험",
            limit=5, document_id=None, use_rerank=False,
            settings=get_settings(), embedder=embedder, store=candidate_store,
            reranker=None,
        )
